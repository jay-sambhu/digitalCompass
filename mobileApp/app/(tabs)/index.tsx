...
export default function IndexScreen() {
  ...
  const { openDrawer, openUserGuide } = useLocalSearchParams<{
    openDrawer?: string;
  ...
  const [locationPerm, setLocationPerm] =
  ...
  const refreshMediaPerm = async () => {
    try {
      ...
      console.error(
        "[Permission Error] Failed to refresh media permission:",
        e?.message ?? "Unknown error",
      );
      return mediaPerm;
    }
  ...
  const openCompassCamera = async (compassIndex: number) => {
    try {
      ...
      const granted = await requestCameraWithFeedback(requestCamPerm, () => {
        setActiveCompass(compassIndex);
        setCapturedPhoto(null);
        setIsCompositePhoto(false);
        setCameraOpen(true);
      ...
      console.error(
        "[Camera Error] Failed to open camera:",
        e?.message ?? "Unknown error",
      );
            ...
            try {
              if (compositeRef.current && compositeRef.current.capture) {
                ...
                if (compositeUri) {
                  ...
                  console.warn(
                    "Composite capture returned no URI, falling back to raw photo",
                  );
                  finishCapture(photo.uri, false);
                }
              } else {
                console.warn(
                  "Composite ref not ready, falling back to raw photo",
                );
                finishCapture(photo.uri, false);
              }
            } catch (compErr: any) {
  ...
  const capturePreviewWithOverlay = async () => {
    ...
    if (previewShotRef.current?.capture) {
      try {
        ...
        console.warn(
          "Preview capture failed, falling back to raw photo:",
          e?.message,
        );
      }
    }
  ...
  const savePhoto = async () => {
    try {
      ...
      try {
        ...
        console.log(
          "Media library error (expected in Expo Go):",
          mediaError.message,
        );
        Alert.alert(
          "Info",
          "Photo captured! (Full save requires development build)",
        );
        setCameraOpen(false);
        setCapturedPhoto(null);
        setIsCompositePhoto(false);
  ...
  const shareApp = async () => {
    try {
      const result = await Share.share({
        message:
          "Check out Sanskar Compass - Vastu Compass App! Download now: https://sanskarvastu.com",
  ...
  const openPermissionsManager = async () => {
    try {
      ...
      console.error(
        "[Permission Error] Failed to open permissions manager:",
        e?.message ?? "Unknown error",
      );
      Alert.alert(
        "Error",
  ...
  const handleCameraPermission = async (value: boolean) => {
    try {
      ...
      console.error(
        "[Permission Error] Camera permission toggle failed:",
        e?.message ?? "Unknown error",
      );
      Alert.alert(
        "Error",
        "Failed to update camera permission. Please try again.",
      );
    }
  };
  ...
  const handleLocationPermission = async (value: boolean) => {
    try {
      if (value && !locationPerm?.granted) {
        await requestLocationWithFeedback(async () => {
      ...
      console.error(
        "[Permission Error] Location permission toggle failed:",
        e?.message ?? "Unknown error",
      );
      Alert.alert(
        "Error",
  ...
  const handleMediaLibraryPermission = async (value: boolean) => {
    try {
      if (value && !mediaPerm?.granted) {
        await requestMediaLibraryWithFeedback(async () => {
          await refreshMediaPerm();
        });
      ...
      console.error(
        "[Permission Error] Media library permission toggle failed:",
        e?.message ?? "Unknown error",
      );
      Alert.alert(
        "Error",
        "Failed to update media library permission. Please try again.",
      );
    }
  };
  ...
  return (
    <SafeAreaView
      style={[
        styles.container,
        { backgroundColor: isDarkMode ? "#f5f5f5" : "#1a1a2e" },
      ]}
      edges={["top"]}
    >
      <ScrollView
      ...
      >
        ...
        >
          ...
          <Text
            style={[
              styles.appTitle,
              { color: isDarkMode ? "#ffffff" : "#BD202E" },
            ]}
          >
            Sanskar Compass
          </Text>
        ...
        >
          <Text
            style={[
              styles.sectionTitle,
              { color: isDarkMode ? "#BD202E" : "#f5f5f5" },
            ]}
          >
        ...
        <View style={styles.compassGrid}>
          {[
            {
              label: "Normal Compass",
              route: "/(tabs)/compass" as Href,
              type: "normal",
            },
            {
              label: "16 Zone Vastu\nCompass",
              route: "/(tabs)/compass2" as Href,
              type: "zone16",
            },
            {
              label: "32 Zone Vastu\nCompass",
              route: "/(tabs)/compass3" as Href,
              type: "zone32",
            },
            {
              label: "SanskarVastu\nCharka",
              route: "/(tabs)/compass4" as Href,
              type: "chakra",
            },
          ].map((item) => {
            ...
            return (
              ...
              >
                ...
                <Text style={[styles.compassLabel, { color: "#ffffff" }]}>
                  {item.label}
                </Text>
              </Pressable>
            );
        ...
        <View style={styles.bottomButtons}>
          <Pressable
            style={[
              styles.bottomBtn,
              { backgroundColor: "#BD202E", borderColor: "#a01b26" },
            ]}
          ...
          <Pressable
            ...
            onPress={() => openWebsite("https://sanskarvastu.com/consultancy")}
          >
            <Text style={[styles.bottomBtnText, { color: "#ffffff" }]}>
        ...
        <View style={styles.bottomButtons}>
          ...
          <Pressable
            ...
            onPress={() => openWebsite("https://sanskarvastu.com/contact")}
          >
            <Text style={[styles.bottomBtnText, { color: "#ffffff" }]}>
              Contact Us
            </Text>
            ...
            <View style={styles.previewContainer}>
                    ...
                    <View style={styles.fullScreenOverlay}>
                      <View style={styles.compassOverlay}>
                        <Image
                          source={
                            getCompassAssets(
                              getCompassTypeByIndex(activeCompass),
                            ).dial
                          }
                          style={{
                      ...
                      <View
                        style={[
                          styles.directionBreakdown,
                          {
                            marginBottom: hasNavigationBar
                              ? insets.bottom + 70
                              : 70,
                          },
                        ]}
              ...
              <View
                style={[
                  ...
                  { paddingBottom: cameraControlsPaddingBottom },
                ]}
              >
              ...
              <CameraView ref={cameraRef} style={styles.camera} facing={facing}>
                <View style={styles.fullScreenOverlay}>
                  <View style={styles.compassOverlay}>
                    <Image
                      source={
                        getCompassAssets(getCompassTypeByIndex(activeCompass))
                          .dial
                      }
                      style={{
                        width: overlayDialSize,
      ...
      >
        <SafeAreaView style={styles.userGuideContainer}>
          <View style={styles.userGuideHeader}>
            <Text style={styles.userGuideTitle}>
              How to Use Sanskar Compass
            </Text>
          ...
          >
              ...
              >
                ...
                <Text
                  style={[
                    ...
                    { color: "#1565C0", fontSize: 18 },
                  ]}
                >
                  What's New - Enhanced Safety!
                </Text>
              </View>
                ...
                <Text style={[styles.guideListItem, { marginBottom: 8 }]}>
                  <Text style={[styles.guideListText, { color: "#0D47A1" }]}>
                    <MaterialIcons
                      ...
                      color="#4CAF50"
                    />{" "}
                    <Text style={{ fontWeight: "700" }}>Never Crashes:</Text>{" "}
                    Advanced error protection prevents all permission-related
                    crashes
                  </Text>
                </Text>
                <Text style={[styles.guideListItem, { marginBottom: 8 }]}>
                  <Text style={[styles.guideListText, { color: "#0D47A1" }]}>
                    ...
                    All features gracefully degrade if you deny permissions
                  </Text>
                </Text>
                <Text style={[styles.guideListItem, { marginBottom: 8 }]}>
                  <Text style={[styles.guideListText, { color: "#0D47A1" }]}>
                    <MaterialIcons
                      ...
                      color="#4CAF50"
                    />{" "}
                    <Text style={{ fontWeight: "700" }}>Smart Fallbacks:</Text>{" "}
                    Automatically uses alternative sensors when permissions
                    denied
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={[styles.guideListText, { color: "#0D47A1" }]}>
                    <MaterialIcons
                      ...
                      color="#4CAF50"
                    />{" "}
                    <Text style={{ fontWeight: "700" }}>Clear Messages:</Text>{" "}
            ...
            <View style={styles.guideSection}>
              ...
              <Text style={styles.guideText}>
                This app helps you find directions and apply Vastu Shastra
                (ancient science of architecture) to your home, office, or any
                space. It's like having a compass and Vastu expert in your
            ...
            <View style={styles.guideSection}>
              ...
              >
                ...
                <Text style={styles.guideSectionTitle}>
                  Permissions - Smart & Safe
                </Text>
              </View>
              <Text style={styles.guideSubtitle}>
                To give you the best experience, we request a few permissions:
              </Text>
              <View style={styles.guideList}>
                <View
                  style={{
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={[
                        styles.guideListText,
                        { fontSize: 12, color: "#666" },
                      ]}
                    >
                      Take photos with compass directions overlaid. If denied,
                      compass still works - you just can't take photos.
                    </Text>
                  </View>
                </View>
                <View
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      Improves compass accuracy with true heading. If denied,
                      app uses magnetometer sensors - compass still works!
                    </Text>
                  </View>
                </View>
                <View
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      Save compass photos to your gallery. If denied, you can
                      still share photos without saving.
                    </Text>
                  </View>
                </View>
              </View>
              ...
              >
                <Text
                  style={[styles.guideText, { fontSize: 13, color: "#2E7D32" }]}
                >
                  <MaterialIcons
                    name="check-circle"
                    size={14}
                    color="#4CAF50"
                  />{" "}
                  <Text style={{ fontWeight: "600" }}>Good News:</Text> The app
                  works even if you deny all permissions! Basic compass
                  functionality always available.
                </Text>
              </View>
              <Text style={styles.guideText}>
                Manage permissions anytime: Menu (☰) → "Manage Permissions".
                You can toggle them on/off as needed.
              </Text>
            </View>
            ...
            <View style={styles.guideSection}>
              ...
              <View style={styles.guideList}>
                <View style={{ marginBottom: 16 }}>
                  ...
                  >
                    A basic compass showing the 4 main directions (North, South,
                    East, West). Perfect for everyday use - just like the
                    compass you learned about in school!
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 16 }}>
                  ...
                  >
                    Divides your space into 16 sections. Good for checking basic
                    Vastu for rooms like bedroom, kitchen, or living room. Great
                    for beginners!
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 16 }}>
                  ...
                  >
                    Divides your space into 32 detailed sections. For advanced
                    Vastu analysis of your entire property. Recommended for
                    serious Vastu practitioners or consultants.
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 8 }}>
                  ...
                  >
                    A special energy wheel used in Sanskar Vastu science. Shows
                    energy flow patterns in your space. Perfect for Vastu
                    experts and energy healers.
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.guideSection}>
              ...
              <View style={styles.guideSteps}>
                <Text style={styles.guideStep}>
                  <Text style={{ fontWeight: "700" }}>1.</Text> On the main
                  screen, tap on any compass card (Normal, 16 Zone, 32 Zone, or
                  Chakra)
                </Text>
                <Text style={styles.guideStep}>
                  <Text style={{ fontWeight: "700" }}>2.</Text> The compass will
                  open - hold your phone flat like a tray
                </Text>
                <Text style={styles.guideStep}>
                  <Text style={{ fontWeight: "700" }}>3.</Text> The red needle
                  points to North. Rotate yourself (not the phone) to face any
                  direction
                </Text>
                <Text style={styles.guideStep}>
                  <Text style={{ fontWeight: "700" }}>4.</Text> Tap the camera
                  icon to take a photo with the compass overlay
                </Text>
                <Text style={styles.guideStep}>
                  <Text style={{ fontWeight: "700" }}>5.</Text> Your photo will
                  be saved with directions marked - perfect for sharing or Vastu
                  analysis!
                </Text>
              </View>
            </View>
            ...
            <View style={styles.guideSection}>
              <Text style={styles.guideSectionTitle}>
                <MaterialIcons name="star" size={18} color="#000" /> Cool
                Features
              </Text>
              <View style={styles.guideList}>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="explore" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>Live Direction:</Text>{" "}
                    Watch the needle move as you turn - shows where you're
                    facing right now
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ...
                    See your exact GPS coordinates (those long numbers are your
                    location on Earth!)
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="battery-full" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>Signal Strength:</Text>{" "}
                    Shows how strong the magnetic field is around you
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="photo-camera" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>Save Photos:</Text> Take
                    pictures with compass directions on them - save and share
                    easily
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="share" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>Share App:</Text> Love
                    the app? Share it with friends and family from the menu
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="shield" size={14} color="#4CAF50" />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      Smart Error Protection:
                    </Text>{" "}
                    Never crashes! Built-in safety handles permission errors
                    gracefully
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="settings" size={14} color="#FF9800" />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      Easy Permission Control:
                    </Text>{" "}
                    Toggle permissions on/off anytime from the menu
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="layers" size={14} color="#2196F3" />{" "}
                    <Text style={{ fontWeight: "700" }}>Sensor Fusion:</Text>{" "}
                    Uses gyroscope + accelerometer + magnetometer for maximum
                    accuracy
                  </Text>
                </Text>
              </View>
            ...
            <View style={styles.guideSection}>
              ...
              <View style={styles.guideList}>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Hold phone flat and steady (like holding a plate of food)
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Stay away from metal things, magnets, speakers, or
                    electronic devices
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Works best outside or near windows (better GPS signal)
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Wait 3-5 seconds for the needle to stop moving before
                    taking a photo
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Compass acting weird? Wave your phone in a figure-8
                    pattern to reset it
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ✓ Grant location permission for best accuracy (but compass
            ...
            <View style={styles.guideSection}>
              ...
              <Text style={[styles.guideSubtitle, { marginTop: 8 }]}>
                Permission Issues:
              </Text>
              <View style={styles.guideList}>
                <View style={{ marginBottom: 12 }}>
                  <Text style={[styles.guideListText, { fontWeight: "600" }]}>
                    <MaterialIcons name="block" size={14} color="#F44336" /> App
                    asks for permission but I denied it:
                  </Text>
                  <Text
                    style={[
                      styles.guideListText,
                      { marginLeft: 0, marginTop: 4, fontSize: 13 },
                    ]}
                  >
                    Don't worry! The app will continue working with reduced
                    features. You can grant permission later from Menu → Manage
                    Permissions or your phone's Settings → Apps → Sanskar
                    Compass → Permissions.
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 12 }}>
                  ...
                  >
                    This is normal and won't crash the app! We've built smart
                    error handling. The app automatically switches to backup
                    sensors when permissions are denied. Just tap "Open
                    Settings" in the alert to enable permissions if you want
                    full features.
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 12 }}>
                  <Text style={[styles.guideListText, { fontWeight: "600" }]}>
                    <MaterialIcons name="warning" size={14} color="#FF5722" />{" "}
                    Can't save photos even with permission:
                  </Text>
                  <Text
                    style={[
                      styles.guideListText,
                  ...
                  >
                    If you're using Expo Go on Android, media library isn't
                    supported. Use the "Share" button instead to send photos to
                    other apps. For full functionality, ask your developer for a
                    development build.
                  </Text>
                </View>
              </View>

              <Text style={[styles.guideSubtitle, { marginTop: 16 }]}>
                Compass Issues:
              </Text>
              <View style={styles.guideList}>
                <View style={{ marginBottom: 12 }}>
                  <Text style={[styles.guideListText, { fontWeight: "600" }]}>
                    <MaterialIcons
                      ...
                      color="#2196F3"
                    />{" "}
                    Needle spinning or jumping around:
                  </Text>
                  <Text
                    style={[
                      styles.guideListText,
                  ...
                  >
                    Move away from magnetic interference (metal furniture,
                    speakers, chargers). Calibrate your phone's compass by
                    moving it in a figure-8 pattern in the air.
                  </Text>
                </View>

                <View style={{ marginBottom: 12 }}>
                  <Text style={[styles.guideListText, { fontWeight: "600" }]}>
                    <MaterialIcons name="gps-off" size={14} color="#9E9E9E" />{" "}
                  ...
                  >
                    Enable location permission for true heading accuracy.
                    Without it, the compass uses magnetic north which can vary
                    by location. Also ensure you're holding the phone flat, not
                    tilted.
                  </Text>
                </View>
                ...
                <View style={{ marginBottom: 12 }}>
                  ...
                  >
                    Grant camera permission when asked. If you accidentally
                    denied it, go to Menu → Manage Permissions and toggle Camera
                    ON. If still not working, check phone Settings → Apps →
                    Sanskar Compass → Permissions.
                  </Text>
                </View>
              </View>
              ...
              >
                <Text
                  style={[styles.guideText, { fontSize: 13, color: "#E65100" }]}
                >
                  <MaterialIcons name="info" size={14} color="#FF9800" />{" "}
                  <Text style={{ fontWeight: "600" }}>Remember:</Text> Even if
                  something goes wrong, the app won't crash! We've added
                  advanced error protection so you can always use the basic
                  compass features.
                </Text>
              </View>
            </View>
            ...
            <View style={styles.guideSection}>
              ...
              >
                ...
                />
                <Text style={styles.guideSectionTitle}>
                  Which Compass Should I Use?
                </Text>
              </View>
              <View style={styles.guideList}>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="home" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      For checking room direction:
                    </Text>{" "}
                    Use Normal Compass
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="weekend" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      For basic Vastu (furniture placement):
                    </Text>{" "}
                    Use 16 Zone Compass
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    <MaterialIcons name="apartment" size={14} color="#000" />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      For complete home/office Vastu:
                    </Text>{" "}
                    Use 32 Zone Compass
                  </Text>
                </Text>
                <Text style={styles.guideListItem}>
                  <Text style={styles.guideListText}>
                    ...
                    />{" "}
                    <Text style={{ fontWeight: "700" }}>
                      For energy and chakra analysis:
                    </Text>{" "}
                    Use SanskarVastu Chakra
                  </Text>
                </Text>
              </View>
            </View>
            ...
            <View style={styles.guideSection}>
              ...
              <Text style={styles.guideText}>
                Want a professional Vastu consultant to analyze your property?
                We offer personalized Vastu services! Just tap on "Vastu
                Consultancy Services" button on the home screen or contact us
                through the menu. Our experts will guide you step by step.
              </Text>
            </View>
            ...
            <View style={styles.guideSection}>
              <Text style={styles.guideSectionTitle}>
                <MaterialIcons name="chat" size={18} color="#000" /> Have
                Questions?
              </Text>
              <Text style={styles.guideText}>
                Confused about something? Want to suggest a new feature? Tap the
                menu (☰) and select "Send Feedback". We read every message and
                reply quickly - usually within 24 hours!
              </Text>
            </View>
            ...
            >
              ...
              >
                <MaterialIcons name="favorite" size={18} color="#000" /> Thank
                you for using Sanskar Compass!{"\n"}
                <Text style={{ fontWeight: "400", fontSize: 13 }}>
                  May your space bring prosperity and peace
                </Text>
              </Text>
            </View>
          </ScrollView>
        ...
        <SafeAreaView style={styles.permissionsContainer}>
          ...
          <ScrollView style={styles.permissionsList}>
            ...
            <View style={styles.permissionItem}>
              <View style={styles.permissionInfo}>
                <MaterialIcons
                  ...
                  size={32}
                  color="#000"
                  style={styles.permissionIconStyle}
                />
                <View style={styles.permissionTextContainer}>
                  <Text style={styles.permissionName}>Camera</Text>
                  <Text style={styles.permissionDesc}>
                    Required for capturing photos with compass overlay
                  </Text>
                </View>
              </View>
            ...
            <View style={styles.permissionItem}>
              <View style={styles.permissionInfo}>
                ...
                <View style={styles.permissionTextContainer}>
                  ...
                  <Text style={styles.permissionDesc}>
                    Required for getting coordinates and compass direction
                  </Text>
                </View>
              </View>
              <View style={styles.permissionToggle}>
            ...
            <View style={styles.permissionItem}>
              <View style={styles.permissionInfo}>
                ...
                <View style={styles.permissionTextContainer}>
                  ...
                  <Text style={styles.permissionDesc}>
                    Required for saving photos to gallery
                  </Text>
                </View>
              </View>
              <View style={styles.permissionToggle}>
            ...
            <Text style={styles.permissionsNote}>
              Toggle ON to grant permissions. Toggle OFF opens device settings
            ...
            <Text style={styles.permissionsInfo}>
              Note: To revoke permissions, use device settings. Granting
              permissions can be done directly from toggles above.
            </Text>
          </ScrollView>
        </SafeAreaView>
      </Modal>
